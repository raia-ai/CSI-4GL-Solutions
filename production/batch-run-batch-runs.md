...
- In Batch Run Menu > Function Parameter Maintenance you can configure the 4GL functions that would execute during the batch run process. Additionally, you would configure the appropriate options for each function. This is usually completed by the Implementation team or copied over from an existing environment.
...
- Go into Developer mode by entering “#” in the 4GL shortcode field. **Note** this can only be done in DEV environments.
