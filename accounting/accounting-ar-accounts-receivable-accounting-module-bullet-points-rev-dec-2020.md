---
title: "4GL Solutions: TECHNOLOGY FOR THE METAL INDUSTRY"
source: "accounting-ar-accounts-receivable-accounting-module-bullet-points-rev-dec-2020.md"
tags: ['SM3', 'Accounting']
version: "1.0"
last_updated: "2026-02-19"
short_description: "4GL Solutions: TECHNOLOGY FOR THE METAL INDUSTRY"
long_description: "This document provides detailed information about 4gl solutions: technology for the metal industry in the 4GL system. It includes procedures, reference information, and best practices for users and administrators."
---

title: "Accounting-AR_-_Accounts_Receivable-Accounting_Module_Bullet_Points_-_Rev_Dec_2020"
source: "Accounting-AR_-_Accounts_Receivable-Accounting_Module_Bullet_Points_-_Rev_Dec_2020.pdf"
tags: ["4GL Solutions", "STEEL MANAGER III", "ERP", "Accounting", "Accounts Receivable", "Accounts Payable", "General Ledger", "Metal Industry", "Financial Management"]
version: "1.0"
last_updated: "2025-12-04"
short_description: "This document outlines the features and functionalities of the accounting modules within the STEEL MANAGER III ERP system, specifically focusing on Accounts Receivable, Accounts Payable, and the General Ledger for the metal industry."
long_description: "A detailed guide to the accounting capabilities of 4GL Solutions' STEEL MANAGER III software, tailored for the metal industry. The document provides a comprehensive overview of the Accounts Receivable (AR) module, covering credit control, customer history inquiries, invoicing, and collections processes. It then details the Accounts Payable (AP) module, explaining how it handles vendor invoices, payments, landed costs, accruals, and purchase variances. The General Ledger (GL) section describes its role as the central repository for all financial transactions, highlighting features like the integrated cash book, bank reconciliation, multi-currency support, custom report generation, and drill-down capabilities. The document concludes with an extensive list of standard reports available across the AR, AP, and GL modules, demonstrating the system's robust reporting and analysis tools for managing a company's financial health."
---
...
*(This figure shows the user interface for maintaining customer master data, including name, address, credit terms, and contact information.)*

### Tracks All Changes to AR & AP
- All changes to fields in Accounts Payable and Accounts Receivable will be tracked as to who made the change, when it was made with the old and new values.

**Figure: Account Changes Tracking Screen**
*(This figure displays a log of changes made to a customer account, showing the field name (e.g., CREDIT LINE), the old value, and the new value, along with the user, date, and time of the change.)*

### Credit Check
Once a sales order is created it is then passed through a credit check. SM3 Credit Checks include:
- Is the Customer over the Credit Limit?
...
**Figure: SM3 Task Manager**
*(This figure shows the Task Manager screen, which lists orders requiring credit approval with details such as document number, account, status, and reason for the hold.)*
...
**Figure: Customer AR Inquiry**
*(This figure displays a comprehensive A/R inquiry screen for a single customer, showing credit limits, balances, aging details, and a list of all open documents like invoices and credit memos.)*

Notes can be entered against an invoice for follow-up purposes.

### Credit Notes
Credit notes can be issued to customers for a variety of reasons. These include over invoice, price adjustment and inventory returns. Each reason (specified in the credit note), can be associated with a specific GL code which is useful for return analysis. If credit notes make reference to an invoice, the system will check to see that the credit value does not exceed the original invoice amount. Once a credit has been entered it is put into a "hold" status and a "credit authorization" form is printed. Credit/Debit Notes can have additional authorizations – the Task Manager informs the Approval authorities of open documents awaiting approval. Once material is returned, any inventory will be returned and the credit amount will be posted to Accounts Receivable. When returning inventory, it is possible to revalue the inventory based on its salability.

### Payments
Payments can be received by check, post-dated check, electronic funds transfer, credit card and cash. Miscellaneous checks can also be entered for non-accounts receivable accounts. During the entry process it will be possible to apply amounts to specific outstanding documents or to post it "on account" if the document is not specified at the time. Once all entries have been entered and balanced, they can then be processed into the AR file. A deposit slip report can be printed as a back-up.

**Figure 10.3: Cash Receipt Entry**
*(This figure shows the screen for entering cash receipts, listing multiple checks received from a customer that make up a single deposit.)*

### Unapplied Documents
A helpful tool available in the AR system is the “unapplied documents" maintenance screen. Here any un-applied documents are displayed and can be easily linked to any other documents if they are known. This generally keeps the aging in a "clean" state.

## ACCOUNTS PAYABLE

### Introduction
The Accounts Payable module will track vendor invoices and payments. It will also track accruals where material has been received, but no vendor invoice has been linked to it yet. This will allow companies to have an accurate analysis of monies owed, and will prevent overpayment or duplicate payment of vendor invoices.

### Highlights
The Accounts Payable module is where you perform functions related to Vendors (adding and maintaining), tracking Vendor invoices and issuing Checks for payment and/or Electronic Payments (no check issued). You can enter and process invoices and disbursements, reconcile checks, perform Accounts Payable period ends, maintain Vendor master files, and administrate the Accounts Payable process. The Accounts Payable module also provides the ability to track accruals – Inventory items have been received, awaiting receipt (link) to Vendor invoice.
...
- **Option to distribute variances from material and outside processing invoices.** SM3 will automatically create a cost adjustment for in stock material or if sold will adjust the cost of sales on the invoice
...
### How SM3 handles Accruals in General Ledger
...
**2. At time of entering/posting AP Voucher (screen AP21B)**
(Assume Vendor Invoice is $10 higher than PO Price - total Invoice Amount $1,010.00)

| Accruals | | A/P | | Price Variance | |
|---|---|---|---|---|---|
| **Dr** | **Cr** | **Dr** | **Cr** | **Dr** | **Cr** |
| 1,000.00 | | | 1,010.00 | 10.00 | |
...
**Options**
**2b. If SM3 is set to automatically distribute Variances to Inventory or Cost of Goods Sold**

| Variance | | Inventory* | | OR | COGS* | |
|---|---|---|---|---|---|---|
| **Dr** | **Cr** | **Dr** | **Cr** | | **Dr** | **Cr** |
| | 10.00 | 10.00 | 0.00 | | 10.00 | 0.00 |

*(partial if stock not 100% sold)*
***No taxes are taken into consideration with above examples**

### Vendor Invoices
Vendor invoices are entered into the system when they are received. The system will perform a check on the invoice number to ensure that no duplicates are entered. The GL debit account(s) to be affected will default from the vendor type which would have been previously assigned to the vendor.

**Figure: Accounts Payable Vendor Payables Inquiry**
*(This figure shows an inquiry screen listing all vouchers for a specific vendor, with details like invoice number, date, currency, open balance, and paid amount.)*

Drilling down into any of the Voucher numbers will provide details of the vendor invoice as follows.

**Figure: Vendor Invoice Details**
*(This figure shows the detail of a single vendor invoice, breaking down the total amount into its GL components, such as Accruals and taxes.)*

If the vendor is an inventory vendor, one of the default accounts will be the accrual account. In such cases, the software will force the entry operator to find and select the appropriate accrual record that was previously generated by the receiving module. This accrual may easily be found through a number of different searches methods. Once the accrual record has been selected, the receipt details will be displayed allowing the operator to check the receipt values against the vendor invoice values. If there is a difference that exceeds the acceptable variance percent, the operator will be issued with a warning.

**Figure 11.1: Vendor Invoice Ref. To Receipt Details**
*(This figure displays a comparison screen between the original receipt details (quantity, price) and the billed details from the vendor invoice, highlighting any variance.)*

STEEL MANAGER III also caters for non-inventory purchases orders and payments.

Vendor invoices can be reversed from the system. By doing this, opposite entries to the original invoice will be posted and the accrual will be reinstated.

### Payments
Payments can be processed in a few different ways.
1. The invoices can be selected from an open payable list in which case they will be added to a batch for check writing.
2. Direct disbursements can be utilized, which gives the ability to process a check (“quick Check”) directly from the system crediting a bank account and debiting an expense/accrual account(s).
...
A rolling TB can be extracted directly into Excel. This is an extremely powerful, time saving feature that will allow easy analysis of quarterly, bi-annual, and annual numbers.

**Figure 12.1: Sample Excel Spreadsheet**
*(This figure shows an example of a rolling trial balance exported to Excel, with GL accounts listed in rows and monthly balances in columns.)*
...
| Category | Report Name | Report Code |
|---|---|---|
...
| **GL Reports** | Trial Balance Report | GL73 |
...
| **Financial Report Generator** | Financial Report File Maintenance | RG1 |
| *(Customers have ability to design their own Financial Reports)* | Financial Report Parameter Listings | RG2 |
...
| **Cash Receipts** | Cash Receipt Register | AR71 |
...
| **Check Reconciliation** | Returned Check Proof Listing | AP62 |
