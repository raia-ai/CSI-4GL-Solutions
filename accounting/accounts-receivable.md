---
title: >-
  Accounting-AR_-_Accounts_Receivable-Accounting_Module_Bullet_Points_-_Rev_Dec_2020
source: >-
  Accounting-AR_-_Accounts_Receivable-Accounting_Module_Bullet_Points_-_Rev_Dec_2020.pdf
version: 1
last_updated: 2025-12-04T00:00:00.000Z
short_description: >-
  This document outlines the features and functionalities of the accounting
  modules within the STEEL MANAGER III ERP system, specifically focusing on
  Accounts Receivable, Accounts Payable, and the General Ledger for the metal
  industry.
long_description: >-
  A detailed guide to the accounting capabilities of 4GL Solutions' STEEL
  MANAGER III software, tailored for the metal industry. The document provides a
  comprehensive overview of the Accounts Receivable (AR) module, covering credit
  control, customer history inquiries, invoicing, and collections processes. It
  then details the Accounts Payable (AP) module, explaining how it handles
  vendor invoices, payments, landed costs, accruals, and purchase variances. The
  General Ledger (GL) section describes its role as the central repository for
  all financial transactions, highlighting features like the integrated cash
  book, bank reconciliation, multi-currency support, custom report generation,
  and drill-down capabilities. The document concludes with an extensive list of
  standard reports available across the AR, AP, and GL modules, demonstrating
  the system's robust reporting and analysis tools for managing a company's
  financial health.
---

# Accounting AR Accounts Receivable Accounting Module Bullet Points Rev Dec 2020
...
New accounts will be faxed or emailed a credit application form and “new customer package" straight from SM3. Once completed and returned, these documents can be electronically attached to the customer profile for safe storage and easy retrieval. **Figure 10.1: Customer File Maintenance** _(This figure shows the user interface for maintaining customer master data, including name, address, credit terms, and contact information.)_

#### Tracks All Changes to AR & AP

* All changes to fields in Accounts Payable and Accounts Receivable will be tracked as to who made the change, when it was made with the old and new values. **Figure: Vendor Invoice Details** _(This figure shows the detail of a single vendor invoice, breaking down the total amount into its GL components, such as Accruals and taxes.)_ If the vendor is an inventory vendor, one of the default accounts will be the accrual account. In such cases, the software will force the entry operator to find and select the appropriate accrual record that was previously generated by the receiving module. This accrual may easily be found through a number of different searches methods. Once the accrual record has been selected, the receipt details will be displayed allowing the operator to check the receipt values against the vendor invoice values. If there is a difference that exceeds the acceptable variance percent, the operator will be issued with a warning. Once a sales order is created it is then passed through a credit check. 4GL Credit Checks include: - Is the Customer over the Credit Limit?
* Are there any past Due Transactions over XX days?
* Has there been no activity for XX days?
* or whether the customer has been placed on manual Credit Hold? If any one of the checks fails, Sales Order will be “held” and will be transferred to the credit department via Task Manager. The credit department can then do an inquiry into the customer's credit. This is a very informative screen, and will give the user plenty of information with which to contact the customer and negotiate the release of the current and any future orders. Information that is available for the user is as follows: start date, credit line, last sale date and amount, average days to pay, highest credit date and amount, last payment date and amount, sales history, open AR and orders. **Figure 10.3: Cash Receipt Entry** _(This figure shows the screen for entering cash receipts, listing multiple checks received from a customer that make up a single deposit.)_
...
The Accounts Payable module is where you perform functions related to Vendors (adding and maintaining), tracking Vendor invoices and issuing Checks for payment and/or Electronic Payments (no check issued). You can enter and process invoices and disbursements, reconcile checks, perform Accounts Payable period ends, maintain Vendor master files, and administrate the Accounts Payable process. The Accounts Payable module also provides the ability to track accruals – Inventory items have been received, awaiting receipt (link) to Vendor invoice. The Accounts Payable module helps you organize your cash flow efficiently and accurately. With the ability to break down the landed cost and the automatic accrual mechanism, you're able to accurately validate vendor invoices and track any purchase variances.
...
**1. At time of Purchase Order / Outside Processing Posting of Receipts** (Assume a Receipt Landed cost of $1000)
...
| 1,010.00                                                                                       |        |             | 1,010.00 |
| **Options**                                                                                    |        |             |          |
| **2b. If 4GL is set to automatically distribute Variances to Inventory or Cost of Goods Sold** |        |             |          |
| Variance                                                                                       |        | Inventory\* |          |
| ---                                                                                            | ---    | ---         | ---      |
| **Dr**                                                                                         | **Cr** | **Dr**      | **Cr**   |
|                                                                                                | 10.00  | 10.00       | 0.00     |
| _(partial if stock not 100% sold)_                                                             |        |             |          |
| \***No taxes are taken into consideration with above examples**                                |        |             |          |

#### Vendor Invoices

Vendor invoices are entered into the system when they are received. The system will perform a check on the invoice number to ensure that no duplicates are entered. The GL debit account(s) to be affected will default from the vendor type which would have been previously assigned to the vendor. **Figure 11.1: Vendor Invoice Ref. To Receipt Details** _(This figure displays a comparison screen between the original receipt details (quantity, price) and the billed details from the vendor invoice, highlighting any variance.)_ STEEL MANAGER III also caters for non-inventory purchases orders and payments. Vendor invoices can be reversed from the system. By doing this, opposite entries to the original invoice will be posted and the accrual will be reinstated.
...
General Ledger integration will **simplify your company's entire accounting procedure.** Automation removes the margin of error associated with manual entries. Access to GL modules can only be obtained by an operator who has been granted the necessary security level. \*\*Features include: \*\*
...
| _(Customers have ability to design their own Financial Reports)_ | Financial Report Parameter Listings    | RG2         |
...
### Accounting-AR\_-_Accounts\_Receivable-Accounting\_Module\_Bullet\_Points_-\_Rev\_Dec\_2020
