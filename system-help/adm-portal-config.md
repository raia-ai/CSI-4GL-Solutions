...
long_description: Help documentation for Configuring the Customer Portal in the 4GL system.
---
...
Click [here](https://4glsol.com/sm3-helpdocs/Content/ExternalFiles/SM3%20eCommerce%20Portal%20User%20Guide.pdf) to view the documentation for using the portal to see the customer point of view.

[![Open](https://4glsol.com/sm3-helpdocs/Skins/Default/Stylesheets/Images/transparent.gif)Customer Setup](https://4glsol.com/sm3-helpdocs/Content/Admin/Config/ADM_Portal_config.htm?Highlight=portal)

Set up the customer’s eCommerce options:

1. In the [customer](https://4glsol.com/sm3-helpdocs/Content/Admin/Config/FM_FL_Util/FM_AR_Cust_Mtce_AR17.htm) file \[AR17], press F4 in the eCommercefield.
2. Indicate if you want to hide any of the following on quote confirmations and quote-to-sales order conversions:
3. Hide Logs? (this hides the Inventory column entirely on the Create Quotes page of the portal)
4. Hide Instock Quantity?
5. Hide Incoming Quantity?
6. Incoming Logs For In-Stock Calculation on quotes (Include, Exclude, or use Warehouse switch settings).
7.  Select the Pricing Method for Non Stock Quantity. During create/modify/convert to order, if the available\* quantity is less than the requested quantity, if you have selected:
...
4. Language, if the contact uses a language different from the default customer language.
5. Web Login user name and Web Password (the Web Login name must be unique across all customers).
6. The warehouses they can access (Whse List) separated by commas (leave blank for all), and the default Whse when the user creates quotes in the portal.
7. Which Web Modules they can see.
8. Optionally, open the Product Lines window to identify specific product lines visible to this user in the portal; if no product line is selected, then all will be visible to the user. Here you can also overwrite the default Non-Stock Pricing Method (see #3 above) for each product line accessible to the user.
...
[![Open](https://4glsol.com/sm3-helpdocs/Skins/Default/Stylesheets/Images/transparent.gif)System Setup](https://4glsol.com/sm3-helpdocs/Content/Admin/Config/ADM_Portal_config.htm?Highlight=portal)
...
[![](https://4glsol.com/sm3-helpdocs/Content/Resources/Images/ADMIN_portal_config_TK12.png)](https://4glsol.com/sm3-helpdocs/Content/Resources/Images/ADMIN_portal_config_TK12.png)

#### IN16 (Product Line) > \[select product line] > F9 <a href="#in16" id="in16"></a>
...
* Number of Inactive Days Allowed - Set the number of days of inactivity after which a portal user’s account will be deactivated.

#### IN1ZB (Warehouse/Product E-Comm Setup) <a href="#in1zb" id="in1zb"></a>
...
* Select the Behavior for each product, or click Behavior to set the values for all displayed products at once:
  * Logs - portal user will select inventory at the log level instead of entering values in the Pieces and Quantity fields.
  * Quantity - portal user can only enter Pieces/Quantity
  * Hybrid - portal user can either select logs or enter Pieces/Quantity (but not both).
...
[![](https://4glsol.com/sm3-helpdocs/Content/Resources/Images/ADM_portal_config_MS39.png)](https://4glsol.com/sm3-helpdocs/Content/Resources/Images/ADM_portal_config_MS39.png)
...
[![Open](https://4glsol.com/sm3-helpdocs/Skins/Default/Stylesheets/Images/transparent.gif)Taking the Portal Offline for Maintenance](https://4glsol.com/sm3-helpdocs/Content/Admin/Config/ADM_Portal_config.htm?Highlight=portal)

There may be occasions when the portal must be taken offline to perform system maintenance. This will normally be done by 4GL. If you find the need to do this yourself, contact [4GL Customer Support](mailto:helpdesk@4glsol.com) for assistance with scripts that will:
