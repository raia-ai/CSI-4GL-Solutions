...
10. After the snippet, select the entire line of the placeholder paragraph and paste the Vimeo embed code over it (so the new div is not embedded within a

    ).
11. For webinar topics: after the snippet (and before the video), include a table of the topics and versions (per file prepared by Paul in _Sharepoint: Documents>4GL Solutions>Webinars>2025 Webinar>4GL Solutions Webinar Enhancements.xlsx_; review for typos). Add this in a dropdown (see January 2025 for an example).
