...
long_description: >-
  Help documentation for Understanding How  Handles Sales Taxes in the 4GL
  system.
---
...
provides a table at each level; for each individual entry (e.g. each city in the City table) you can define the default tax rate and whether that level's tax should be applied in _addition_ to a tax at a higher level, and whether the tax is the _last_ tax to be considered (ignoring a tax at a lower level).
