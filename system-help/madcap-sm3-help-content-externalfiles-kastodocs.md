---
title: "KASTO – Computer Link for the Storage Computer"
source: "madcap-sm3-help-content-externalfiles-kastodocs.md"
tags: ['SM3', 'System Help']
version: "1.0"
last_updated: "2026-02-19"
short_description: "KASTO – Computer Link for the Storage Computer"
long_description: "This document provides detailed information about kasto – computer link for the storage computer in the 4GL system. It includes procedures, reference information, and best practices for users and administrators."
---
...
| :--- | :--- | :--- |
| 2019.03.01 | Initial Version | G. Lambertz |
| 2019.06.06 | Correction HLBM/LHBM, Function code 4+6 | G. Lambertz |

---
*Property of KASTO Maschinenbau GmbH & Co. KG. This manual as a whole or in part can only be copied or distributed to a third party with our explicit consent.*
---
...
2.  **Different methods of transmitting data**
    2.1. Transfer of data via files-transfer
        ...
        2.1.4. Process Chart REKO-TCP/IP via passive FTP
    ...
    2.3. Data base interface
        ...
        2.3.2. Reception table Host → KASTOlogic
        2.3.3. Transmitting table KASTOlogic → Host
3.  **Telegram overview**
    3.1. HLQU Acknowledgement telegram LVR -> HOST (option)
    3.2. LHQU Acknowledgement telegram HOST -> LVR (option)
    3.3. HLAU Order - entry
    3.4. LHAU Order - acknowledgement
    3.5. LHBK Inventory correction messages
    3.6. HLBM Inventory inquiry (article)
    3.7. LHBM Inventory feedback (article)
    3.8. HLMS Machine Status maintenance (option)
    3.9. LHMS Machine Status acknowledgement (option)
    3.10. HLSA Master data inquiry
    3.11. HLST Master Data maintenance
    3.12. LHST Master Data feedback
    3.13. HOST reference number

## 1. General notes

### 1.1 Usage
The computer link is the connection between the LVR and the production control system respectively the material scheduling level of the customer's data processing system. It enables data exchange for the following tasks:
- Order processing
...
| :--- | :--- |
| **Telegrams / field contents** | For each type of telegram, some fields are defined. If needed, more fields can be appended, subject to agreement. Fields may only contain printable ASCII-characters. |
...
| :--- | :--- |
| **dhxxxx.ftp** | **For files from HOST to LVR**<br>- for transportation<br>- containing messages and acknowledgements (option) from HOST<br>- will be renamed after being saved at LVR as `dhxxxx.rcv`. |
| **dhxxxx.rcv** | **For files from HOST to LVR**<br>- was renamed from `dhxxxx.ftp`.<br>- is entered at the LVR and then deleted. |
| **dlyyyy.ftp** | **For files from LVR to HOST**<br>- for transportation.<br>- containing messages and acknowledgements (option) from LVR.<br>- will be renamed after being saved at HOST as `dlyyyy.ksd`. |
| **dlyyyy.ksd** | **For files from LVR to HOST**<br>- was renamed from `dlyyyy.ftp`.<br>- is entered at the HOST and then deleted. |
| **xxxx and yyyy** | **Dummy for a number**<br>- runs in cycles from 0000 to 9999.<br>- is increased when creating a new file (at Host or LVR).<br>- characteristic to distinguish several simultaneously existing files. |
...
**HOST to LVR**
1.  **HOST:** Creates and transmits file `dhxxxx.ftp`.
...
**(Option 'Acknowledgement') LVR to HOST**
1.  **LVR:** Creates acknowledgement file `dlyyyy.ftp` (with telegram type `LHQU`) in `d:\host\tcpsnd` and transmits it.
2.  **HOST:** Saves `dlyyyy.ftp` in its directory.
3.  **LVR:** Renames `dlyyyy.ftp` to `dlyyyy.ksd` using FTP command "rename".
4.  **HOST:** Reads and then erases `dlyyyy.ksd`.

**LVR to HOST (Data Processing)**
1.  **LVR:** After processing, creates file `dlyyyy.ftp` (e.g., with telegram type `LHAU`) in `d:\host\tcpsnd` and transmits it.
2.  **HOST:** Saves `dlyyyy.ftp` in its directory.
3.  **LVR:** Renames `dlyyyy.ftp` to `dlyyyy.ksd` using FTP command "rename".
4.  **HOST:** Reads and then erases `dlyyyy.ksd`.

**(Option 'Acknowledgement') HOST to LVR**
...
**(Option 'Acknowledgement') LVR to HOST**
1.  **LVR:** Creates acknowledgement file `dlyyyy.ksd` (with telegram type `LHQU`) in `d:\host\tcpsnd`.
2.  **HOST:** After a time buffer (e.g., 1 min), fetches `dlyyyy.ksd` from `d:\host\tcpsnd`.
3.  **HOST:** Saves `dlyyyy.ksd` in its directory.
...
3.  **HOST:** Saves, reads, and then erases `dlyyyy.ksd` from `d:\host\tcpsnd` using FTP command "delete".
...
1.  **HOST -> KASTOlogic:** `Data (SEQ: 000001)`
2.  **KASTOlogic -> HOST:** `QU (FE: 0, SEQ: 000001)` (No error)
3.  **HOST -> KASTOlogic:** `Data (SEQ: 000002)`
4.  **KASTOlogic -> HOST:** `QU (FE: 0, SEQ: 000002)` (No error)

**2.2.4.3.2 Repeat data telegram**
1.  **HOST -> KASTOlogic:** `Data (SEQ: 000003)`
2.  *(No acknowledgement received by HOST within timeout)*
3.  **HOST -> KASTOlogic:** `Data (SEQ: 000003)` (Repetition)
4.  **KASTOlogic -> HOST:** `QU (FE: 0, SEQ: 000003)` (No error)

**2.2.4.3.3 Data telegram with unexpected sequence no.**
1.  **HOST -> KASTOlogic:** `Data (SEQ: 000004)`
2.  **KASTOlogic -> HOST:** `QU (FE: 0, SEQ: 000004)` (No error)
3.  **HOST -> KASTOlogic:** `Data (SEQ: 000006)` (Unexpected sequence)
4.  **KASTOlogic -> HOST:** `QU (FE: 2, SEQ: 000006)` (Error: sequence no. Next expected SEQ: 000005)
...
| Table | User | Rights |
| :--- | :--- | :--- |
| **T_KASTO_RCV** | "rekohost" (Host-user) | INSERT |
| | KASTO-user | SELECT, DELETE |
| **T_KASTO_SND** | "rekohost" (Host-user) | SELECT, DELETE |
| | KASTO-user | INSERT |

*There is also the possibility to access each table with a different user.*

#### 2.3.2 Reception table Host → KASTOlogic
**Table: T_KASTO_RCV**
| Column | Data Type | Description |
| :--- | :--- | :--- |
| `rcv_serial` | NUMBER(10) DEFAULT 0 | Unique counter |
| `rcv_status` | NUMBER(1) DEFAULT 0 | Will not be interpreted |
| `rcv_dat` | DATE DEFAULT SYSDATE | Insert date on data base computer |
| `rcv_data` | VARCHAR2(2000) NOT NULL | Data/telegram filed by Host system |

New data base records are created by data base user "rekohost". The KASTOlogic-data base user checks in regular intervals for new data records.

#### 2.3.3 Transmitting table KASTOlogic → Host
**Table: T_KASTO_SND**
| Column | Data Type | Description |
| :--- | :--- | :--- |
| `snd_serial` | NUMBER(10) DEFAULT 0 | Unique counter |
| `snd_status` | NUMBER(1) DEFAULT 0 | Will not be interpreted |
| `snd_dat` | DATE DEFAULT SYSDATE | Insert date of data base computer |
| `snd_data` | VARCHAR2(2000) NOT NULL | Data/telegram filed by Host system |

New data base records are created by KASTOlogic. The "rekohost"-data base user checks for new data records regularly.
...
| :--- | :--- | :--- |
| O_Ack | Yes(X) or No(-) | Acknowledgement telegrams (upon agreement) |
| O_Charge | - | Charge (heat) management |
| O_Cutsaw | - | Interface of KASTO-saws (fully-integrated) |
| O_Machine | - | Machine status |
| O_Pack | - | Pack-information during commissioning |
| O_Prod | - | Cutting optimization / production |
| O_Reserv | - | Reservation of inventories |
| O_Sheetcom | - | Automatic commissioning of sheets |

### Telegram Overview Table

| Telegram type | Project-specific use | KASTOlogic with Inventory admin. | KASTOlogic with Cassette number admin. | Meaning | Source | Destination |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| HLQU | O_Ack | O_Ack | Acknowledgement telegram LVR -> HOST | HOST | LVR |
| LHQU | O_Ack | O_Ack | Acknowledgement telegram HOST -> LVR | LVR | HOST |
| HLAA | X | | | Order modification | HOST | LVR |
| LHAA | X | | | Order modification feedback | LVR | HOST |
| HLAT | X | | | article type | HOST | LVR |
| LHAT | X | | | article type feedback | LVR | HOST |
| HLAU | X | | | Order-entry | HOST | LVR |
| LHAU | X | | | Order-acknowledgement | LVR | HOST |
| LHBK | X | | | Inventory correction messages | LVR | HOST |
| HLBM | X | | | Inventory inquiry(article) | HOST | LVR |
| LHBM | X | | | Inventory feedback (article) | LVR | HOST |
| HLCD | O_Charge | | | Create/modify charge base data | HOST | LVR |
| LHCD | O_Charge | | | Charge data answer | LVR | HOST |
| LHFT | O_Prod | | | Manufacturing Parts Message | LVR | HOST |
| HLFU | X | | | Cassette Field Overview - Request | HOST | LVR |
| LHFU | X | | | Cassette Field Overview - Return message | LVR | HOST |
| HLKL | | X | X | Cassette Listing - Request | HOST | LVR |
| LHKL | | X | X | Cassette Listing - Return message | LVR | HOST |
| LHKP | | | X | Cassette Position - Return message | LVR | HOST |
| HLKS | X | X | Cassette-Master Data | HOST | LVR |
| LHKS | X | X | Cassette-Master Data-Return message | LVR | HOST |
| HLKT | X | | | Create manual stock - Request | HOST | LVR |
| LHKT | X | | | Create manual stock - Return message | LVR | HOST |
| HLKV | | | X | Cassette number administration - request | HOST | LVR |
| LHKV | | | X | Cassette number administration - feedback | LVR | HOST |
| HLMS | O_Machine | O_Machine | | Machine Status maintenance | HOST | LVR |
| LHMS | O_Machine | O_Machine | | Machine Status acknowledgment | LVR | HOST |
| LHPK | O_Pack | | | Pack-Information-Return message | LVR | HOST |
| HLRE | O_Reserv | | | Inventory Reservation | HOST | LVR |
| LHRE | O_Reserv | | | Inventory Reservation-Return message | LVR | HOST |
| HLSA | X | | | Master Data inquiry | HOST | LVR |
| HLSP | O_Prod | | | Cutting layout create/modify/remove | HOST | LVR |
| LHSP | O_Prod | | | Cutting layout feedback | LVR | HOST |
| HLST | X | | | Master Data maintenance | HOST | LVR |
| LHST | X | | | Master Data feedback | LVR | HOST |
| HLTD | O_Cutsaw | | | Technology master data maintenance | HOST | LVR |
| LHTD | O_Cutsaw | | | Technology master data feedback | LVR | HOST |
| HLTA | O_Cutsaw | | | Technology master data request | HOST | LVR |
| HLUM | | | X | Cassette movement order - request | HOST | LVR |
| LHUM | | | X | Cassette movement order - feedback | LVR | HOST |

### 3.1 HLQU Acknowledgement telegram LVR -> HOST (option)
**Telegram-Type: HLQU (HOST → LVR)**
Each telegram from LVR → HOST (except `LHQU`) is acknowledged by HOST via this telegram.

| Field No. | Description | Length | Type | Reqd Field | Comment |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | Telegram-type | 4 | alpha | yes | `HLQU` |
| 2 | LVR-telegram-type | 4 | alpha | yes | Content of field "Telegram-Type" from telegram LVR → HOST |
| 3 | LVR-sequence n° | 6 | num | yes | Content of field "Sequence no." from telegram LVR → HOST |
| 4 | LVR-function | 2 | num | yes | Content of field "Function" from telegram LVR → HOST |
| 4b | Host-Ref-No. | 20 | alpha | no | Option: Content of field "Host-Ref-No" from telegram LVR → HOST |
| 5 | Tele-form-fault-code | 2 | num | yes | `0`: No fault, telegram received correctly<br>`1`: Telegram received with fault<br>`2`: Sequence nº fault, next sequence nº see field 6 (With socket only) |
| 6 | Next sequence nº | 6 | num | no | With socket only: HOST expects this nº as next sequence nº from LVR. (Required if fault-code == 2) |
| | **Total length:** | | | | |
| | EOS | | | | End-Of-String: `\n` or LF (file) |

### 3.2 LHQU Acknowledgement telegram HOST -> LVR (option)
**Telegram-Type: LHQU (LVR → HOST)**
Each telegram from HOST → LVR (except `HLQU`) is acknowledged by LVR via this telegram.

| Field No. | Description | Length | Type | Reqd Field | Comment |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | Telegram-type | 4 | alpha | yes | `LHQU` |
| 2 | HOST-telegram-type | 4 | alpha | yes | Content of field "Telegram-Type" from telegram HOST → LVR |
| 3 | HOST-sequence no | 6 | num | yes | Content from field "Sequence no." from telegram HOST → LVR |
| 4 | HOST-Function | 2 | num | yes | Content of filed "Function" from telegram HOST → LVR |
...
### 3.3 HLAU Order - entry
**Telegram-Type: HLAU (HOST → LVR)**

| Field No. | Description | Length | Type | Reqd Field | Default | Comment |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | Telegram-Type | 4 | alpha | yes | | `HLAU` |
| 2 | Sequence no. | 6 | num | yes | | |
| 3 | Function | 2 | num | yes | | `1`: New order or change of order. An existing order with the same order nº and position is changed if not yet in process.<br>`2`: Erase order. |
| 3b | Host ref no. | (20) | alpha | no | | Host-reference number (Optional field) |
...
### 3.4 LHAU Order - acknowledgement
**Telegram-Type: LHAU (LVR → HOST)**

| Field No. | Description | Length | Type | Comment |
| :--- | :--- | :--- | :--- | :--- |
| 1 | Telegram-Type | 4 | alpha | `LHAU` |
| 2 | Sequence n°. | 6 | num | |
| 3 | Fault-Code | 2 | num | `0`: Order worked correctly<br>`1`: Order erased manually<br>`2`: Commissioning order back partially ready; or intermediate message on bar change (Option KASTOcenter)<br>`3`: Cassette out of system in production (intermediate)<br>`4`: Cassette on station (intermediate)<br>`5`: Order has been started (Option)<br>`6`: Order removed because missing material<br>*Order rejected if fault code > 50.* |
| 3b | Host ref no. | (20) | alpha | Host-Reference No. (optional field) |
| 4 | Order-Type | 2 | num | |
| 5 | Urgent order | 1 | num | `0` = no, `1` = yes |
| 6 | Order n°. | 10 | alpha/num | |
| 7 | Order-position | 10 | alpha/num | |
| 8 | Article | 24 | alpha | |
| 9 | Amount unit | 3 | alpha | |
| 10 | Heat | 24 | alpha | |
...
### 3.5 LHBK Inventory correction messages
**Telegram-Type: LHBK (LVR → HOST)**

| Field No. | Description | Length | Type | Comment |
| :--- | :--- | :--- | :--- | :--- |
| 1 | Telegram-Type | 4 | alpha | `LHBK` |
| 2 | Sequence no | 6 | num | |
| 3 | Fault-Code | 2 | num | `0`: Inventory correction to order<br>`1`: Manual inventory correction (from dialogue)<br>`2`: Manual correction via booking of a manual order<br>`11`: Automatically inventory correction (added)<br>`12`: Automatically inventory correction (removal) |
| 4 | Article | 24 | alpha | |
| 5 | Amount Unit | 3 | alpha | |
| 6 | Heat | 24 | alpha | |
| 7 | Heat2 | 24 | alpha | |
| 8 | Cassette-no | 10 | num | |
| 9 | Compartment | 5 | num | |
| 10 | Amount Added | 12.4 | num | |
| 11 | Amount Removal | 12.4 | num | |
| 12 | Piece Added | 10 | num | Only with optional piece management |
| 13 | Piece Removal | 10 | num | Only with optional piece management |
| 14 | Reason | 10 | alpha | reason of manual change correction |
| 15 | Info | 32 | alpha | Subcompartment-Info |
| 16 | Storage Length | 12.4 | num | |
| 17 | Storage width | 12.4 | num | (if article type 'sheet' or 'plate') |
| 18 | Heat3 | 24 | alpha | Info field |
| 19 | Fieldname | 6 | alpha | Fieldname |
...
### 3.6 HLBM Inventory inquiry (article)
**Telegram-Type: HLBM (HOST → LVR)**
LVR answers to HLBM with: `LHBM`.

| Field No. | Description | Length | Type | Reqd Field | Default | Comment |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | Telegram Type | 4 | alpha | yes | | `HLBM` |
| 2 | Sequence no. | 6 | num | yes | | |
| 3 | Function | 2 | num | yes | | `1`: All articles: Inventories of all articles are returned.<br>`2`: One article: Inventory of article 'Article-1' is returned.<br>`3`: Area: Inventories from 'Article-1' to 'Article-2' are returned.<br>`4`: One article: Inventory by Heat1 (SUM/unique) is returned.<br>`5`: All articles: Inventories with amount > 0 are returned.<br>`6`: All articles: Inventories with amount > 0 are returned per Heat1.<br>`7`: One article: Inventories returned by compartments.<br>`8`: All articles: Inventories returned by compartments.<br>`9`: All articles: Inventory returned per bar.<br>`10`: One article: Inventory of 'Article-1' returned per bar. |
| 3b | Host ref no. | (20) | alpha | no | | Host-Reference No.(optional field) |
| 4 | Article 1 | 24 | alpha | no | | |
| 5 | Article 2 | 24 | alpha | no | | |
| 6 | Article type | 10 | alpha | no | | With functions 1, 3, 5, 6, 8 and 9. |
...
### 3.7 LHBM Inventory feedback (article)
**Telegram-Type: LHBM (LVR → HOST)**

| Field No. | Description | Length | Type | Comment |
| :--- | :--- | :--- | :--- | :--- |
| 1 | Telegram-Type | 4 | alpha | `LHBM` |
| 2 | Sequence no | 6 | num | |
| 3 | Fault-Code | 2 | num | `0`: no fault<br>`51`: function unknown<br>`61`: article unknown<br>`62`: inventory not identified |
| 3b | Host ref no. | (20) | alpha | Host-Reference No.(optional field) |
| 4 | Article | 24 | alpha | |
| 5 | Amount Unit | 3 | alpha | |
| 6 | Date of reset | 8 | num | Date (Format YYYYMMDD), when following fields of article (field 4) have been set to zero |
| 7 | Added Quantity | 12.4 | num | Added quantity, since last 'Date of reset' |
| 8 | Removal amount | 12.4 | num | Removal amount, since last 'Date of reset' |
| 9 | Access counter | 10 | num | Accesses, since last 'Date of reset' |
| 10 | Heat | 24 | alpha | With function 4, 6, 7, 8, 9, 10 only |
| 11 | Heat2 | 24 | alpha | With function 7, 8, 9, 10 only |
| 12 | Stored length | 12.4 | num | With function 7 or 8 only |
| 13 | Amount | 12.4 | num | |
| 14 | Info | 32 | alpha | With function 7 or 8 only |
| 15 | Cassette no | 10 | num | N° of cassette |
| 16 | Compartment no | 5 | num | No of compartment |
...
### 3.8 HLMS Machine Status maintenance (option)
**Telegram type: HLMS (HOST → LVR)**

| Field no. | Description | Length | Type | Comment / admissible values |
| :--- | :--- | :--- | :--- | :--- |
| 1 | Telegram type | 4 | Alpha | `HLMS`. Machine type and no. is an explicit requirement. |
| 2 | Sequence no. | 6 | Num | |
| 3 | Function | 2 | Num | `1`: Set Status: Set status or permission for distribution.<br>`2`: Status-Request: Request on status or permission for distribution. |
...
### 3.10 HLSA Master data inquiry
**Telegram-Type: HLSA (HOST → LVR)**
LVR answers to HLSA with: `LHST`.

| Field No | Description | Length | Type | Reqd Field | Comment |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | Telegram-Type | 4 | alpha | yes | `HLSA` |
| 2 | Sequence n°. | 6 | num | yes | |
...
### 3.11 HLST Master Data maintenance
**Telegram-Type: HLST (HOST → LVR)**
On fault, LVR answers to HLST with: `LHST`.

| Field No. | Description | Length | Type | Reqd Field | Default | Comment |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | Telegram-Type | 4 | alpha | yes | | `HLST` |
| 2 | Sequence no | 6 | num | yes | | |
